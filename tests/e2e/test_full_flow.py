...
def genre(auth_client):
    """Create a genre."""
    response = auth_client.post('/api/v1/genres/', {
...
def book(auth_client, genre):
    """Create a book."""
    response = auth_client.post('/api/v1/books/', {
        'title': 'Dune',
...
class TestE2EUserFlow:
    ...
    def test_register_login_profile(self, anon_client, db):
        # 1. Register
        reg = anon_client.post('/api/v1/users/register/', {
            'username': 'flow_user',
            'email': 'flow@test.com',
        ...
        # 2. Login
        login = anon_client.post('/api/v1/users/login/', {
            'username': 'flow_user',
            'password': 'flowpass123',
        })
    ...
    def test_token_refresh(self, anon_client, db):
        anon_client.post('/api/v1/users/register/', {
        ...
        refresh = anon_client.post('/api/v1/users/token/refresh/', {
...
class TestE2EBookFlow:
    ...
    def test_book_search_filter(self, auth_client, genre):
        auth_client.post('/api/v1/books/', {
            'title': 'Django for APIs', 'author': 'William Vincent',
            'publication_date': '2022-01-01', 'genre': genre['id'],
            'copies_total': 3, 'copies_available': 3,
        })
        auth_client.post('/api/v1/books/', {
            'title': 'Flask Web Development', 'author': 'Miguel Grinberg',
            'publication_date': '2018-01-01', 'genre': genre['id'],
            'copies_total': 2, 'copies_available': 2,
        })
    ...
    def test_genre_filter(self, auth_client):
        ...
        auth_client.post('/api/v1/books/', {
            ...
            'publication_date': '1937-09-21', 'genre': g1.data['id'],
            'copies_total': 4, 'copies_available': 4,
        })
        auth_client.post('/api/v1/books/', {
            'title': 'Gone Girl', 'author': 'Flynn',
            'publication_date': '2012-06-05', 'genre': g2.data['id'],
            'copies_total': 2, 'copies_available': 2,
        })
...
class TestE2EReservationFlow:
    ...
    def test_cannot_reserve_no_copies(self, auth_client, genre):
        book_resp = auth_client.post('/api/v1/books/', {
            'title': 'Rare Book', 'author': 'Unknown',
            'publication_date': '2020-01-01', 'genre': genre['id'],
            'copies_total': 0, 'copies_available': 0,
        })
        reserve = auth_client.post('/api/v1/reservations/', {'book': book_resp.data['id']})
    ...
    def test_list_only_own_reservations(self, auth_client, book):
        ...
        other_client.post('/api/v1/users/register/', {
            'username': 'other_user', 'email': 'other@test.com',
            'password': 'otherpass123',
        })
        login = other_client.post('/api/v1/users/login/', {
            'username': 'other_user', 'password': 'otherpass123',
        })
...
class TestE2ERatingFlow:
    ...
    def test_rate_book_and_check_analytics(self, auth_client, book):
        ...
        # 1. Rate the book
        rate = auth_client.post(f'/api/v1/books/{book_id}/rate/', {
            'rate': 85,
...
class TestE2EFullJourney:
    ...
    def test_complete_user_journey(self, db):
        ...
        genre = client.post('/api/v1/genres/', {
            'name': 'Programming', 'slug': 'programming',
        })
        assert genre.status_code == 201

        book1 = client.post('/api/v1/books/', {
            'title': 'Clean Code', 'author': 'Robert C. Martin',
            'isbn': '9780132350884', 'publication_date': '2008-08-01',
            'genre': genre.data['id'], 'copies_total': 3, 'copies_available': 3,
        })
        assert book1.status_code == 201

        book2 = client.post('/api/v1/books/', {
            'title': 'Clean Architecture', 'author': 'Robert C. Martin',
            'isbn': '9780134494166', 'publication_date': '2017-09-20',
            'genre': genre.data['id'], 'copies_total': 2, 'copies_available': 2,
        })
        ...
        reservation = client.post('/api/v1/reservations/', {
            'book': book1.data['id'],
        })
        ...
        rate = client.post(f'/api/v1/books/{book1.data["id"]}/rate/', {
            'rate': 92, 'review': 'Must read for every developer',
        })
        assert rate.status_code == 201
        ...
        extend = client.post(f'/api/v1/reservations/{reservation_id}/extend/', {
            'days': 14,
        })
        assert extend.status_code == 200
