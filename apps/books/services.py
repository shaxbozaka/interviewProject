...
class BookRepository:
    ...
    def get_all_with_ratings() -> QuerySet:
        ...
        return Book.objects.annotate(get_rating=Avg("ratings__rate"))
    ...
    def get_by_id(book_id: int) -> Book:
        ...
        trace_step(
            f"BookRepository.get_by_id({book_id}) → CACHE MISS, fetched from DB, cached",
            "db",
        )
        return book
    ...
    def create_rating(book: Book, user, rate: int, review: str = "") -> Rating:
        trace_step(
            f"BookRepository.create_rating(book={book.id}, user={user.id}, rate={rate})",
            "logic",
        )
        try:
        ...
        trace_step(
            f"Celery: update_book_analytics_task.delay(book={book.id}) → RabbitMQ",
            "task",
        )
