...
def fetch_books_for_subject(subject, needed, stdout):
    ...
    while len(books) < needed and offset < 5000:
        ...
        for w in works:
            ...
            edition_key = avail.get("openlibrary_edition") or w.get(
                "cover_edition_key", ""
            )
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for genre in genres:
            ...
            if not real_books:
                self.stdout.write(self.style.WARNING("  Skipping - no results"))
                continue
            ...
            for i, book_id in enumerate(book_ids):
                ...
                if dry_run:
                    ...
                    self.stdout.write(
                        f"  #{book_id}: {rb['title']} — {rb['author']}{desc_flag}{read_flag}"
                    )
                    updated += 1
                else:
                    try:
        ...
        if not dry_run:
            self.stdout.write(
                "Run: python manage.py reindex_books  (to rebuild search index)"
            )
