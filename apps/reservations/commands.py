...
class ReserveBookCommand:
    ...
    def validate(self):
        trace_step(f'ReserveBookCommand.validate() book={self.book.id} user={self.user.id}', 'logic')
    ...
    def execute(self) -> Reservation:
        ...
        self.book.save()
        trace_step(f'DB: Book #{self.book.id} copies_available decremented to {self.book.copies_available}', 'db')
        cache_invalidate_pattern('books')
        trace_step('Cache: invalidated "books" pattern (L1 cleared + Redis keys deleted)', 'cache')
        ...
        trace_step(f'DB: Reservation #{reservation.id} created (status=ACTIVE, due={self.loan_days}d)', 'db')
        send_reservation_confirmation.delay(self.user.id, self.book.title)
        trace_step(f'Celery: send_reservation_confirmation.delay(user={self.user.id}) → RabbitMQ', 'task')
        publish_event(
            topic='book-events',
        ...
        trace_step(f'Celery: update_book_analytics_task.delay(book={self.book.id}) → RabbitMQ', 'task')
        AuditLog.objects.create(
            ...
            changes={
                ...
                'event': 'book.reserved',
                'pipeline': ['kafka:book-events', 'celery:send_reservation_confirmation', 'celery:update_book_analytics'],
            },
        )
...
class ReturnBookCommand:
    ...
    def validate(self):
        trace_step(f'ReturnBookCommand.validate() reservation={self.reservation.id}', 'logic')
    ...
    def execute(self) -> Reservation:
        ...
        trace_step(f'DB: Book #{self.reservation.book.id} copies_available incremented to {self.reservation.book.copies_available}', 'db')
        cache_invalidate_pattern('books')
        ...
        trace_step(f'Celery: send_return_confirmation.delay(user={self.reservation.user.id}) → RabbitMQ', 'task')
        publish_event(
            topic='book-events',
        ...
        trace_step(f'Celery: update_book_analytics_task.delay(book={self.reservation.book.id}) → RabbitMQ', 'task')
        AuditLog.objects.create(
            ...
            changes={
                ...
                'event': 'book.returned',
                'pipeline': ['kafka:book-events', 'celery:send_return_confirmation', 'celery:update_book_analytics'],
            },
        )
...
class ExtendReservationCommand:
    ...
    def validate(self):
        trace_step(f'ExtendReservationCommand.validate() reservation={self.reservation.id}', 'logic')
    ...
    def execute(self) -> Reservation:
        ...
        self.reservation.save()
        trace_step(f'DB: Reservation #{self.reservation.id} due_date extended +{self.extra_days}d', 'db')
        return self.reservation
