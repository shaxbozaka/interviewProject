...
class TestReservationAPI:
    def test_create_reservation(self, authenticated_client, user):
        ...
        response = authenticated_client.post(
            url, {"book": book.pk, "due_date": "2024-02-01T00:00:00Z"}
        )
    ...
    def test_list_only_own_reservations(self, api_client):
        ...
        Reservation.objects.create(
            user=user1, book=book, status="active", due_date="2024-02-01T00:00:00Z"
        )
        Reservation.objects.create(
            user=user2, book=book, status="active", due_date="2024-02-01T00:00:00Z"
        )
    ...
    def test_duplicate_active_reservation_fails(self, authenticated_client, user):
        ...
        response = authenticated_client.post(
            url, {"book": book.pk, "due_date": "2024-03-01T00:00:00Z"}
        )
        assert response.status_code == 400
