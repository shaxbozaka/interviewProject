...
def profile_cache():
    ...
    print(
        f"Cache write ({iterations} ops): {write_time * 1000:.2f}ms ({write_time / iterations * 1000:.3f}ms/op)"
    )
    ...
    print(
        f"Cache read L1 ({iterations} ops): {l1_time * 1000:.2f}ms ({l1_time / iterations * 1000:.3f}ms/op)"
    )
    ...
    print(
        f"Cache read L2 ({iterations} ops): {l2_time * 1000:.2f}ms ({l2_time / iterations * 1000:.3f}ms/op)"
    )
...
def profile_trie():
    ...
    print(
        f"Search (1,000 lookups): {search_time * 1000:.2f}ms ({search_time / 1000 * 1000:.3f}ms/op)"
    )
    ...
    print(
        f"Autocomplete (1,000 prefix searches): {auto_time * 1000:.2f}ms ({auto_time / 1000 * 1000:.3f}ms/op)"
    )
...
if __name__ == "__main__":
    if len(sys.argv) < 2 or sys.argv[1] not in COMMANDS:
        print("Usage: python scripts/benchmark.py <command>")
