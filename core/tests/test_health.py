...
class TestHealthCheck:
    def test_healthy_when_all_services_up(self, api_client):
        with (
            patch(
                "core.health._check_kafka",
                return_value={"status": "up", "latency_ms": 5.0},
            ),
            patch(
                "core.health._check_elasticsearch",
                return_value={"status": "up", "latency_ms": 3.0},
            ),
        ):
    ...
    def test_degraded_when_non_critical_down(self, api_client):
        with (
            patch(
                "core.health._check_kafka",
                return_value={"status": "down", "error": "unavailable"},
            ),
            patch(
                "core.health._check_elasticsearch",
                return_value={"status": "down", "error": "unavailable"},
    ...
    def test_includes_all_service_statuses(self, api_client):
        with (
            patch(
                "core.health._check_kafka",
                return_value={"status": "down", "error": "n/a"},
            ),
            patch(
                "core.health._check_elasticsearch",
                return_value={"status": "down", "error": "n/a"},
